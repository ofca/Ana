...
class Validation implements \ArrayAccess 
{
    ...
    public function check()
    {
        ...
        foreach ($rules as $field => $set)
        {
            ...
            foreach ($set as $array)
            {
                ...
                elseif (method_exists('\Ana\Valid', $rule))
                {
                    // Use a method in this object
                    $method = new \ReflectionMethod('\Ana\Valid', $rule);
    ...
    public function errors($file = null)
    {
        ...
        foreach ($this->_errors as $field => $set)
        {
            ...
            if ( ! isset(self::$messages[$file]) and is_file($filePath)) {
                self::$messages[$file] = $messages = include $filePath;
            }

            if (isset(self::$messages[$file]) and isset(self::$messages[$file][$field])) {
                $message = self::$messages[$file][$field][$error];
